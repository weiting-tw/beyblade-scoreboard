...
typedef struct {
  ...
  bey_sr_event_cb user_cb;
  void *user_cb_arg;
  bey_sr_fill_cb fill_cb;
  void *fill_cb_arg;
...
void bey_sr_handler_task(void *pvParam) {
  while (true) {
    bey_sr_result_t result;
    ...
    if (WAKENET_DETECTED == result.wakenet_mode) {
      if (g_sr_data->user_cb) {
        g_sr_data->user_cb(g_sr_data->user_cb_arg, BEY_SR_EVENT_WAKEWORD, -1, -1);
      }
      continue;
    }
    ...
    if (WAKENET_CHANNEL_VERIFIED == result.wakenet_mode) {
      if (g_sr_data->user_cb) {
        g_sr_data->user_cb(g_sr_data->user_cb_arg, BEY_SR_EVENT_WAKEWORD_CHANNEL, result.command_id, -1);
      }
      continue;
    }
    ...
    if (ESP_MN_STATE_DETECTED == result.state) {
      if (g_sr_data->user_cb) {
        g_sr_data->user_cb(g_sr_data->user_cb_arg, BEY_SR_EVENT_COMMAND, result.command_id, result.phrase_id);
      }
      continue;
    }
    ...
    if (ESP_MN_STATE_TIMEOUT == result.state) {
      if (g_sr_data->user_cb) {
        g_sr_data->user_cb(g_sr_data->user_cb_arg, BEY_SR_EVENT_TIMEOUT, -1, -1);
      }
      continue;
    }
  }
...
static void audio_detect_task(void *arg) {
  ...
  while (true) {
    ...
    if (g_sr_data->mode == BEY_SR_MODE_WAKEWORD) {
      if (res->wakeup_state == WAKENET_DETECTED) {
        ...
        bey_sr_set_mode(BEY_SR_MODE_OFF);
    ...
    if (g_sr_data->mode == BEY_SR_MODE_COMMAND) {
...
esp_err_t bey_sr_start(
  ...
  g_sr_data->result_que = xQueueCreate(3, sizeof(bey_sr_result_t));
  ...
  afe_config.wakenet_model_name = esp_srmodel_filter(models, ESP_WN_PREFIX, "nihaoxiaozhi");
  ...
  ret_val = xTaskCreatePinnedToCore(&bey_sr_handler_task, "SR Handler Task", 6 * 1024, NULL, configMAX_PRIORITIES - 1, &g_sr_data->handle_task, 1);
  //ret_val = xTaskCreatePinnedToCore(&bey_sr_handler_task, "SR Handler Task", 6 * 1024, NULL, configMAX_PRIORITIES - 1, &g_sr_data->handle_task, 0);
