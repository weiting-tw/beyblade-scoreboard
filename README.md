...
| 按鍵 | 接法 | 韌體能不能用 |
| --- | --- | --- |
| `BOOT` | GPIO0（上拉，按下為 0） | **可以**。正式韌體用它在計分頁重來這一局 |
| `PWR` | 純硬體電源電路，不接 GPIO | **不行** |

`PWR` 的路徑（Rev1.1 原理圖）：按鍵 → `U6`（ECJ23001-4FCBD6，單鍵開關機控制
IC，圖上標 `POWER ON/OFF`）→ `PWR_EN` → `Q4`/`Q5` MOSFET → `PWR_N_EN` →
`U2` 與 `RT9193-33PB`（3.3V LDO）的 EN 腳。整條路徑沒有任何 GPIO，所以韌體
讀不到它，也不需要拉住某支腳來維持供電（有些 Waveshare 板子是 `SYS_EN` 那種
設計，沒實作會一開機就斷電；這塊不是）。

**開關機**：關機狀態下短按開機，開機狀態下長按約 6 秒關機。**只在電池供電時
有意義** —— MOSFET 切的是電池那一路，USB 接著時 VBUS 直接供電，按了不會關機。

要重新確認腳位用 `pio run -e button_probe -t upload`：它把候選 GPIO 全設成
上拉輸入，按下去哪支變 0 就是哪支。
